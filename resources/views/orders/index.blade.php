<x-app-layout>
    <x-slot name="header">
        <span class="inline-block bg-portal-accent text-black px-2.5 py-1 rounded-md text-[0.7rem] font-extrabold uppercase tracking-wider mb-2">{{ __('My Procurement') }}</span>
        <h1 class="text-4xl font-extrabold tracking-tight mb-2">{{ __('Purchase Orders') }}</h1>
        <p class="text-portal-muted text-lg">{{ __('Track and manage your material requisitions.') }}</p>
    </x-slot>
    ...
    <div class="bg-portal-sidebar border border-portal-border rounded-xl flex flex-col min-h-[600px]">
        <div class="p-6 border-b border-portal-border flex items-center justify-between">
            <h2 class="font-display font-bold text-lg">{{ __('Transaction History') }}</h2>
            <div class="flex items-center gap-3">
                <div class="relative">
                    <x-lucide-search class="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-portal-muted" />
                    <input type="text" placeholder="{{ __('Search PO number...') }}" class="bg-white/5 border border-portal-border rounded-lg ps-9 pe-4 py-2 text-sm text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted w-64">
                </div>
                ...
                <table class="w-full text-start">
                    <thead class="bg-white/2 text-xs font-bold text-portal-muted uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-8 py-4 text-center">{{ __('Items') }}</th>
