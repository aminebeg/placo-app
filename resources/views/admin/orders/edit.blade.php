<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="orderForm()">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight uppercase italic text-white flex items-center gap-3">
                    ...
                    <span class="text-lg bg-white/10 px-2 py-1 rounded text-portal-muted not-italic font-mono">#{{ $order->order_number }}</span>
                </h1>
                <p class="text-portal-muted mt-1">{{ __('Modification complète de la commande et des informations client associées.') }}</p>
            </div>
            
            <div class="flex items-center gap-3">
                <a href="{{ route('orders.show', $order->id) }}" class="px-4 py-2 rounded-lg border border-portal-border text-portal-muted hover:text-white hover:bg-white/5 transition-colors text-sm font-medium flex items-center gap-2">
                    <x-lucide-eye class="w-4 h-4" /> {{ __('Voir') }}
                </a>
        ...
        <form id="order-edit-form" action="{{ route('admin.orders.update', $order->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            @method('PUT')
                ...
                <div class="bg-portal-sidebar/50 backdrop-blur-sm border border-portal-border rounded-xl overflow-hidden shadow-lg shadow-black/20">
                    <div class="px-6 py-4 border-b border-portal-border bg-white/5 flex items-center justify-between">
                        <h2 class="font-bold text-lg flex items-center gap-2 text-white">
                            <span class="p-1.5 rounded-lg bg-purple-500/10 text-purple-400"><x-lucide-user class="w-5 h-5" /></span>
                            {{ __('Informations Client') }}
                        </h2>
                        <span class="text-xs text-portal-muted bg-white/5 px-2 py-1 rounded">{{ __('Modifie le profil') }}</span>
                    </div>

                    <!-- User Search/Select -->
                    <div class="px-6 pt-6 -mb-2 relative z-20">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider mb-2">{{ __('Rechercher un autre client') }}</label>
                        <div class="relative">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="first_name" value="{{ old('first_name', $order->user->first_name) }}" required 
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-portal-muted"><x-lucide-user class="w-4 h-4" /></span>
                                <input type="text" name="last_name" value="{{ old('last_name', $order->user->last_name) }}" required 
                                    class="w-full bg-black/40 border border-portal-border text-white rounded-lg pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-portal-accent/50 focus:border-portal-accent transition-all placeholder:text-white/20" 
                                    placeholder="Doe">
                            </div>
                        ...
                        <div class="md:col-span-2 group">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="company" value="{{ old('company', $order->user->company) }}" 
                                    class="w-full bg-black/40 border border-portal-border text-white rounded-lg pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-portal-accent/50 focus:border-portal-accent transition-all placeholder:text-white/20" 
                                    placeholder="SARL...">
                            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-portal-sidebar/50 backdrop-blur-sm border border-portal-border rounded-xl overflow-hidden p-6 space-y-6 shadow-lg shadow-black/20">
                     <h3 class="font-bold text-white text-sm uppercase tracking-wider border-b border-white/10 pb-2 mb-4">{{ __('Paramètres de Commande') }}</h3>
                     
                    <div class="relative">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider mb-2">{{ __('Statut Actuel') }}</label>
                        <select name="status" class="w-full bg-black/40 border border-portal-border text-white rounded-lg pl-4 pr-10 py-2.5 focus:ring-1 focus:ring-portal-accent focus:border-portal-accent appearance-none transition-colors">
                            <option value="pending" class="bg-[#141415]" {{ old('status', $order->status) == 'pending' ? 'selected' : '' }}>{{ __('En attente (Pending)') }}</option>
                            <option value="confirmed" class="bg-[#141415]" {{ old('status', $order->status) == 'confirmed' ? 'selected' : '' }}>{{ __('Confirmé (Confirmed)') }}</option>
                            <option value="in_delivery" class="bg-[#141415]" {{ old('status', $order->status) == 'in_delivery' ? 'selected' : '' }}>{{ __('En Livraison (In Delivery)') }}</option>
                            <option value="delivered" class="bg-[#141415]" {{ old('status', $order->status) == 'delivered' ? 'selected' : '' }}>{{ __('Livré (Delivered)') }}</option>
                            <option value="cancelled" class="bg-[#141415]" {{ old('status', $order->status) == 'cancelled' ? 'selected' : '' }}>{{ __('Annulé (Cancelled)') }}</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="requested_delivery_date" value="{{ old('requested_delivery_date', $order->requested_delivery_date) }}" class="w-full bg-black/40 border border-portal-border text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-portal-accent/50 focus:border-portal-accent transition-colors">
                    </div>
                </div>
                ...
                <div class="bg-portal-sidebar/50 backdrop-blur-sm border border-portal-border rounded-xl overflow-hidden p-6 shadow-lg shadow-black/20">
                    ...
                    <textarea name="delivery_address" rows="4" class="w-full bg-black/40 border border-portal-border text-white rounded-lg px-4 py-3 focus:ring-2 focus:ring-portal-accent/50 focus:border-portal-accent transition-colors resize-none">{{ old('delivery_address', $order->delivery_address) }}</textarea>
                </div>

                <!-- Notes -->
                <div class="bg-portal-sidebar/50 backdrop-blur-sm border border-portal-border rounded-xl overflow-hidden p-6 shadow-lg shadow-black/20">
                    <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider mb-2">{{ __('Notes Internes') }}</label>
                    <textarea name="notes" rows="3" class="w-full bg-black/40 border border-portal-border text-white rounded-lg px-4 py-3 focus:ring-2 focus:ring-portal-accent/50 focus:border-portal-accent transition-colors resize-none" placeholder="Instructions...">{{ old('notes', $order->notes) }}</textarea>
                </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('orderForm', () => ({
                items: @json(old('items', $order->items->map(function($item) { return ['product_id' => $item->product_id, 'quantity' => $item->quantity]; }))),
