    ...
    <div class="bg-portal-sidebar border border-portal-border rounded-xl p-8 max-w-4xl">
        <form method="POST" action="<?php echo e(route('admin.products.update', $product->id)); ?>" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-6">
                ...
                <div class="mb-6">
                    ...
                    <?php if($product->image_url): ?>
                        <div class="mb-4">
                            <img src="<?php echo e($product->image_url); ?>" alt="Current Image" class="h-32 w-auto object-contain rounded-lg border border-portal-border bg-white/5 p-2">
                        </div>
                    <?php endif; ?>
                    ...
                    <p class="text-xs text-portal-muted mt-1">Leave blank to keep current image.</p>
                </div>
                
                <div class="grid grid-cols-3 gap-6">
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider">Name (EN)</label>
                        <input type="text" name="name_en" value="<?php echo e(old('name_en', $product->name_en)); ?>" required class="w-full bg-white/5 border border-portal-border rounded-lg px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm font-medium">
                    </div>
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider">Name (FR)</label>
                        <input type="text" name="name_fr" value="<?php echo e(old('name_fr', $product->name_fr)); ?>" required class="w-full bg-white/5 border border-portal-border rounded-lg px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm font-medium">
                    </div>
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider">Name (AR)</label>
                        <input type="text" name="name_ar" value="<?php echo e(old('name_ar', $product->name_ar)); ?>" dir="rtl" required class="w-full bg-white/5 border border-portal-border rounded-lg px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm font-medium">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="price" value="<?php echo e(old('price', $product->price)); ?>" required class="w-full bg-white/5 border border-portal-border rounded-lg pl-8 pr-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm font-mono font-bold">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        ...
                        <textarea name="description_en" rows="4" class="w-full bg-white/5 border border-portal-border rounded-lg px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm"><?php echo e(old('description_en', $product->description_en)); ?></textarea>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider"><?php echo e(__('Description (FR)')); ?></label>
                        <textarea name="description_fr" rows="4" class="w-full bg-white/5 border border-portal-border rounded-lg px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm"><?php echo e(old('description_fr', $product->description_fr)); ?></textarea>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider"><?php echo e(__('Description (AR)')); ?></label>
                        <textarea name="description_ar" rows="4" dir="rtl" class="w-full bg-white/5 border border-portal-border rounded-lg px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 placeholder-portal-muted text-sm"><?php echo e(old('description_ar', $product->description_ar)); ?></textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="number" step="0.001" name="weight_kg" value="<?php echo e(old('weight_kg', $product->weight_kg)); ?>" class="w-full bg-white/5 border border-portal-border rounded-xl px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 text-sm font-mono">
                    </div>
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-portal-muted uppercase tracking-wider">Pieces per Bundle</label>
                        <input type="number" name="pieces_per_bundle" value="<?php echo e(old('pieces_per_bundle', $product->pieces_per_bundle)); ?>" class="w-full bg-white/5 border border-portal-border rounded-xl px-4 py-2.5 text-white focus:border-portal-accent focus:ring-0 text-sm font-mono">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-portal-border">
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="px-6 py-3 rounded-xl font-bold text-sm text-portal-muted hover:text-white hover:bg-white/5 transition-colors">Discard</a>
